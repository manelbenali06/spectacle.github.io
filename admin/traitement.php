<?php
    ...
    if ($valid === true) {
        ...
        move_uploaded_file($_FILES['img']['tmp_name'], '../assets/img/photos/' . $imgName); // upload du fichier
    ...
    if (isset($_GET['delete']) && !empty($_GET['delete'])) {
        $id = (int)$_GET['delete'];
        $req = $db->query('SELECT img FROM cirque WHERE id=' . $id); // récupère le nom de l'image
        $oldImg = $req->fetch();
        if (file_exists('../assets/img/photos/' . $oldImg['img'])) { // vérifie que le fichier existe
            unlink('../assets/img/photos/' . $oldImg['img']); // supprime l'image du dossier local
        }
